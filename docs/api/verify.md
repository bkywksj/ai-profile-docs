...
所以本库只把**零成本验证**（本页）做成通用接口。需要真实调用时：

| 能力 | 做法 |
|---|---|
| 对话 | 用应用自己的对话实现发一句 ping（请求格式、鉴权头都是应用的），`max_tokens` 别给 1 —— 推理模型可能直接报错 |
| 生图 / 配音 | 直接调 [`generate` / `synthesize`](/api/media)，就是试运行 |
| 视频 | 不提供 —— 单次约 ¥1–5、要等 90 秒，让用户在真实业务里验证 |
