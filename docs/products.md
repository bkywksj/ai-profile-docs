---
description: 抓蛙师出品的开发者工具矩阵：覆盖智能编程、凭据安全、服务器运维、知识管理、桌面框架、全栈开发等场景。
---
...
::: tip 与 ai-profile 的关系
标着「内置 ai-profile」的产品，模型服务的预置、验证、限额与 `ai.profile` 互通都由本库提供 ——
在其中一个应用里配好的模型服务，复制一段 `ai.profile` 就能粘进另一个。
:::
...
<AProductCard
  ...
  name="Sigil 掌玺"
  slogan="AI 凭据金库 · MCP 协议代理"
  description="让 AI 帮你执行任务，但 Token、密钥、密码永远不离开本地。OS 密钥环 + AES-256-GCM 双重加密、完整审计、Hook 拦截 —— 凭据明文永不外泄。"
  theme="blue"
  badge="内置 ai-profile"
  :highlights="[
    '凭据金库：13 种凭据类型，AES-256-GCM 加密后落 OS 密钥环，明文用完即从内存清零',
    'MCP 协议原生支持：Claude Code、Cursor、Cline、Zed 等主流客户端零适配接入',
    '160+ 内置能力：Git 多平台、GitHub Actions、对象存储、邮件、MQTT、代码签名…… 危险动作逐次审批',
    '文件加密与加密分享包：磁盘上只有密文；分享包对方浏览器拖进去即可解密',
    '完整审计 + 输出脱敏 Hook：Token 值不入库，明文进出 MCP 前自动占位'
  ]"
  :actions="[
...
<AProductCard
  icon="lucide:rocket"
  logo="/products/ruoyi-plus-uniapp.png"
  name="RuoYi-Plus-UniApp"
  slogan="Spring Boot 3 + Vue 3 + UniApp 企业级全栈框架"
  description="基于成熟开源项目深度重构的企业级全栈快速开发框架，四层架构设计，涵盖后端、前端管理端、移动端。多租户、支付集成、AI能力、代码生成器，减少代码量70%。"
  theme="blue"
  :highlights="[
    '四层架构：Controller-Service-DAO-Mapper，职责清晰分明',
    'Claude Code深度集成：29+专业技能、12个智能命令',
    '企业级能力：多租户、支付、小程序/公众号、AI大模型集成',
    '移动端覆盖：UniApp跨平台，支持微信小程序、H5、App',
    '80+企业信赖，3800+次提交，持续迭代优化'
  ]"
  :actions="[
    { text: '访问官网', link: 'https://ruoyi.plus/' }
  ]"
/>
...
<AProductCard
  ...
  badge="42集教程"
  :highlights="[
    '八大领域63个模块：封面设计、视频制作、PPT/Excel/Word、图片处理、音频合成等',
