    ...
    <div class="product-left">
      <div class="product-left-top">
        <div class="product-icon-wrapper">
          ...
          <!-- lucide:* → 内联图标(替代原 emoji);其它字符串仍按文字渲染以兼容旧写法 -->
          <Icon v-else-if="icon.startsWith('lucide:')" :icon="icon" class="product-icon-svg" />
...
/* lucide 内联图标(替代 emoji)—— 尺寸对齐原 emoji,颜色随卡片主题 */
.product-icon-svg {
  font-size: 30px;
  color: var(--vp-c-brand-1);
}

.product-card.theme-blue .product-icon-svg {
  color: #0b6ef0;
}

.product-card.theme-purple .product-icon-svg {
  color: #8b5cf6;
}

.product-card.theme-green .product-icon-svg {
  color: #10b981;
}
